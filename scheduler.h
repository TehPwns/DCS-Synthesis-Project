...
/* A file defining a list schdueling algorithm */
...
struct user_input 
{
	...
	* of instances. If a key does not exist, assume an upper bound of infinite. */
	...
	//Maximum requested latency of the schduele, or -1 for infinite
	int maxLatency = -1;
};

//Output of the list schduler
struct output
{
	...
	/* Map of string->int specifiying the number of resources of each type allocated
	 * Examples: 
	 *   ADD -> 1; SUB -> 2; MULT -> 2 */
...
/*(P. 208)
 * Given a module to schduele and a sequenceing graph, along with user input 
