...
void make_a_local_copy(int** local_compat, const vvint& compat, int nodesize)
{
    ...
    for(i=0; i<nodesize; i++)
        memcpy(local_compat[i], compat[i].data(), nodesize*sizeof(int));
...
vvint clique_partition(const graph& compat_graph)
{
    ...
    ret.resize(numCliques);

    //Copies result stored in `clique_set` by the partitioner into `ret`
    for(int i=0; i<numCliques; ++i) {
        ...
        for(int j=0; j<MAXCLIQUES; j++) {
            if(clique_set[i].members[j] != UNKNOWN)
                ret[i].push_back(clique_set[i].members[j]);
            else break;
        }
    }
